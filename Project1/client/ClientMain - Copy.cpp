	...
	try {
		...
		while (!my_exiting)
		{
			...
			if (tokens.size() >= 1)
			{
				...
				if (!my_connected && !my_waitingForConnect)
				{
				...
				if (command == "create")
				{
					...
					else
					{
						...
						::ToLower(roomname);
						auto roomID = chat.GetRoom(roomname);
				...
				if (command == "room")
				{
					...
					else
					{
						...
						else
						{
							cCommand::tUID uid = 0;
							unsigned short size = (unsigned short)roomname.size();
							cCommand command(cCommand::eRoomConnect, my_UID, room, roomname.c_str(), size);
							cout << "Sending Room Create command: " << endl;
							cout << command;
							sock->Send(command.GetRawMessage(), command.GetRawLength());
							my_waitingForResponse = true;
